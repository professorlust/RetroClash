    ...
    public class PlayerDb
    {
        ...
        public static async Task<Player> Create()
        {
            try
            {
                ...
                using (var cmd =
                    new MySqlCommand(
                        $"INSERT INTO player (`Id`, `Score`, `Language`, `Avatar`, `GameObjects`) VALUES ({id + 1}, {player.Score}, @language, @avatar, @objects)")
                )
                {
...
#pragma warning restore 618
        ...
        public static async Task Save(Player player)
        {
            try
            {
                using (var cmd =
                    new MySqlCommand(
                        $"UPDATE player SET `Score`='{player.Score}', `Language`='{player.Language}', `FacebookId`=@fb, `Avatar`=@avatar, `GameObjects`=@objects WHERE Id = '{player.AccountId}'")
                )
                {
#pragma warning disable 618
